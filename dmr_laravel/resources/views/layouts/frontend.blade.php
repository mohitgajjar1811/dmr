...
<head>
    ...
    <style>
        ...
        /* CKEditor Content Styling - Ensuring Frontend matches Admin */
        .ck-content table {
...
<body class="flex flex-col min-h-screen bg-gray-50 text-gray-800 overflow-x-hidden"
        ...
        <div class="container mx-auto px-4 flex justify-between items-center text-[10px] md:text-xs relative">
            ...
            <div class="flex items-center gap-6">
                <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $company_info->email ?? 'info@virjaautomation.com' }}"
                    ...
                    <span class="font-medium tracking-wide">{{ $company_info->email ?? 'info@virjaautomation.com'
                        }}</span>
                </a>
            ...
            <!-- Center: Company Name -->
            <div
                class="absolute left-1/2 -translate-x-1/2 text-white font-tech font-bold tracking-wider sm:tracking-widest uppercase text-[8px] sm:text-xs md:text-sm whitespace-nowrap">
                ...
                <div class="flex gap-4">
                    @if($company_info->facebook ?? false)
                        <a href="{{ $company_info->facebook }}" target="_blank"
                            class="hover:text-secondary transition text-white"><i class="fab fa-facebook-f"></i></a>
                    @endif
                    @if($company_info->linkedin ?? false)
                        <a href="{{ $company_info->linkedin }}" target="_blank"
                            class="hover:text-secondary transition text-white"><i class="fab fa-linkedin-in"></i></a>
                    @endif
                    @if($company_info->instagram ?? false)
                        <a href="{{ $company_info->instagram }}" target="_blank"
                            class="hover:text-secondary transition text-white"><i class="fab fa-instagram"></i></a>
                    @endif
                    @if(!($company_info->facebook ?? false) && !($company_info->linkedin ?? false))
                        ...
                        <a href="#" class="hover:text-secondary transition text-white"><i
    ...
    <footer class="bg-gray-900 text-gray-300 pt-10 pb-6 border-t-4 border-secondary">
        <div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
                <h3 class="text-white text-lg font-bold mb-4 uppercase">{{ $company_info->name ?? 'Virja Industries' }}
                </h3>
                <p class="text-sm text-gray-400 leading-relaxed">
                    {{ $company_info->footer_description ?? 'Leading supplier of industrial safety gear.' }}
                </p>
            </div>
            <div>
                ...
                <ul class="space-y-2 text-sm">
                    ...
                    <li><a href="{{ route('contact') }}" class="hover:text-secondary">Contact Us</a></li>
                </ul>
            </div>
            <div>
                <h3 class="text-white text-lg font-bold mb-4 uppercase">Contact</h3>
                <ul class="space-y-4 text-sm">
                    <li><i class="fas fa-map-marker-alt text-secondary mr-3"></i>
                        {{ $company_info->address ?? 'Pune, India' }}</li>
                    <li><i class="fas fa-phone text-secondary mr-3"></i>
                        {{ $company_info->phone ?? '+91 123 456 7890' }}</li>
                </ul>
            </div>
            <div>
                ...
                <form action="#" method="POST" class="flex flex-col space-y-2">
                    ...
                    <input type="email" name="email" placeholder="Email address"
                        class="px-4 py-2.5 rounded bg-gray-800 border border-gray-700 text-white">
                    <button type="submit"
                        class="bg-secondary text-white py-2.5 rounded font-bold hover:bg-red-700">Subscribe</button>
                </form>
            </div>
        </div>
    </footer>
